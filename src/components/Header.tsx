...
import { ChevronDown, ChevronUp, CircleUser, Ellipsis, Headset, Info, LogOut, Search, Settings, X, Zap } from 'lucide-react';
...
export default function Header() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const isCommandK = (e.metaKey && e.key.toLowerCase() === 'k') || (e.ctrlKey && e.key.toLowerCase() === 'k');
      if (isCommandK) {
        e.preventDefault();
  ...
  return (
    <div
      className={cn(
        ...
        'bg-[#0C0C18]/95 backdrop-blur-md z-100 pointer-events-auto',
      ...
      <div className="w-full flex items-center justify-between xl:justify-start gap-3 px-4 h-16 ">
        ...
        <div className="relative w-95 hidden xl:flex">
          <Search size={14} className="text-white/25 absolute top-1/2 -translate-y-1/2 left-3.5 pointer-events-none" />
        ...
        <div className="flex items-center gap-2 xl:hidden">
          ...
          <span className="text-sm font-semibold tracking-tight text-white">Mini CRM</span>
        </div>
      ...
      <div className={cn('w-full hidden xl:flex items-center justify-between xl:justify-end', 'gap-3 px-4 xl:h-[64px]', { flex: openMenu })}>
        {mounted && (
            ...
            <DropdownMenuTrigger asChild>
              <Button
                ...
                <Avatar className="h-8 w-8">
                  <AvatarImage src="" />
                  <AvatarFallback className="bg-gradient-to-br from-violet-500 to-indigo-600 text-white text-xs font-semibold">N</AvatarFallback>
                </Avatar>
                <div className="hidden xl:block text-left">
                  <p className="text-xs font-medium text-white/80 leading-none">Minh Duc</p>
                  <p className="text-[10px] text-white/30 mt-0.5 leading-none">Admin</p>
                </div>
                {isOpenProfile ? <ChevronUp size={13} className="text-white/30" /> : <ChevronDown size={13} className="text-white/30" />}
              </Button>
            </DropdownMenuTrigger>

            <DropdownMenuContent align="end" className="w-[240px] rounded-xl border border-white/[0.08] bg-[#141420] shadow-xl shadow-black/50 p-2">
              <div className="px-3 py-2.5 mb-1">
                <p className="text-sm font-medium text-white/80">Nguyen Minh Duc</p>
                <p className="text-xs text-white/30 mt-0.5">nguyenminhduc@gmail.com</p>
              </div>
