...
import { Customer, GetCustomersListRequest, Group, SortKey } from '@/src/features/customer/types';
import { ColumnHeader, SORT } from '@/src/types';
import { Mail, MailX, Pencil, Search, SlidersHorizontal, Trash2, X } from 'lucide-react';
...
export function CustomerTable({ customers, total, isLoading, request, onRequestChange, onDelete, onEdit }: Props) {
  ...
  const hasActiveFilter = !!(filter.q || filter.groups || filter.has_newsletter);

  const updateFilter = (patch: Partial<NonNullable<GetCustomersListRequest['filter']>>) => {
    onRequestChange({
      ...request,
      filter: { ...filter, ...patch },
  ...
  const handleSort = (key: SortKey<Customer>) => {
    ...
    const newOrder = sameField && request.sort?.order === SORT.DESC ? SORT.ASC : SORT.DESC;
    onRequestChange({
      ...request,
    ...
    () => [
      {
        id: 'id',
        label: '',
        cellRender: row => <Checkbox checked={selectedIds.has(row.id)} onChange={() => toggleRow(row.id)} />,
      },

      {
        ...
        cellRender: row => (
          <div className="w-8 h-8 relative rounded-full overflow-hidden ring-1 ring-white/10 bg-white/5 shrink-0">
            {row.avatar ? (
              <Image sizes="32px" src={row.avatar} alt={row.first_name} fill className="object-cover" />
            ) : (
...

      {
        ...
        cellRender: row => (
          <div>
            <div className="text-white/60 whitespace-nowrap text-xs">{row.city ?? '—'}</div>
            <div className="text-[11px] text-white/25 truncate max-w-[180px]">{row.address ?? ''}</div>
          </div>
        ),
      },
...

      {
        id: 'total_spent',
        label: 'Total Spent',
        cellRender: row => <span className="text-emerald-400 font-medium tabular-nums text-xs">{formatShortNumber(row.total_spent)}</span>,
      },
        ...
        cellRender: row => (
          <span
            className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${
              row.has_ordered ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20' : 'bg-white/5 text-white/25 border border-white/10'
            }`}>
...

      {
        id: 'last_seen',
        label: 'Last Seen',
        cellRender: row => <span className="text-white/30 whitespace-nowrap text-[11px]">{formatDate(row.last_seen)}</span>,
      },

      {
        ...
        label: 'Last Purchase',
        cellRender: row => (
          <span className="text-white/30 whitespace-nowrap text-[11px]">{row.latest_purchase ? formatDate(row.latest_purchase) : '—'}</span>
        ),
      },

      {
        id: 'first_seen',
        label: 'Joined',
        cellRender: row => <span className="text-white/25 whitespace-nowrap text-[11px]">{formatDate(row.first_seen)}</span>,
      },
    ...
    <div className="rounded-xl border border-white/[0.07] bg-overlay w-full">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 px-5 py-3 border-b border-white/6">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-[10px] text-white/20 uppercase tracking-widest mr-1 shrink-0">Group</span>
          <FilterPill label="All" active={!filter.groups} onClick={() => updateFilter({ groups: undefined })} />
          {(['regular', 'ordered_once', 'collector', 'reviewer', 'compulsive'] as Group[]).map(g => (
            <FilterPill
              key={g}
              label={GROUP_LABELS[g]}
              active={filter.groups === g}
              onClick={() => updateFilter({ groups: filter.groups === g ? undefined : g })}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-white/20 uppercase tracking-widest mr-1 shrink-0">Newsletter</span>
          <FilterPill label="All" active={!filter.has_newsletter} onClick={() => updateFilter({ has_newsletter: undefined })} />
          <FilterPill label="Yes" active={filter.has_newsletter === 'true'} onClick={() => updateFilter({ has_newsletter: 'true' })} />
          <FilterPill label="No" active={filter.has_newsletter === 'false'} onClick={() => updateFilter({ has_newsletter: 'false' })} />
        </div>
      </div>
      ...
      {showSort && (
        <div className="flex items-center gap-2 px-5 py-3 border-b border-white/6 flex-wrap">
          ...
          {LIST_SORT_BY.map(({ key, label }) => (
            <SortBtn key={key} colKey={key} label={label} sortKey={sortField} sortDir={sortDir} onSort={handleSort} />
          ))}
        </div>
      )}
      ...
      {selectedIds.size > 0 ? (
        <div className="flex items-center gap-3 px-5 py-2.5 border-b border-white/6 bg-violet-500/6">
          ...
          <span className="text-[11px] text-violet-300 font-medium">{selectedIds.size} selected</span>
        ...
        <div className="flex items-center gap-3 px-5 py-2.5 border-b border-white/4">
          <Checkbox checked={allPageSelected} indeterminate={somePageSelected} onChange={togglePage} />
          <span className="text-[10px] text-white/20 uppercase tracking-widest">Select rows</span>
        </div>
      )}
      ...
      {isLoading ? (
        <div className="py-20 text-center text-white/20 text-xs">Loading customers…</div>
      ) : customers.length === 0 ? (
        <div className="py-20 text-center text-white/20 text-xs">No customers match your filters.</div>
      ) : (
        <CustomTable<Customer> columnHeader={columns} columnData={customers} />
      )}

      <Pagination page={currentPage} totalPages={totalPages} total={total} perPage={perPage} onPageChange={updatePage} />
    </div>
  );
}
