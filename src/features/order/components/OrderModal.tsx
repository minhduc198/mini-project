...
import { SearchableOption, SearchableSelect, SearchableSelectField } from '@/src/components/SearchableSelectField';
...
export function OrderModal(props: Props) {
  const { open, onClose, isSubmitting = false, mode, customers, products } = props;
      ...
      <DialogContent
        ...
        <DialogHeader className="flex-row items-center gap-3 px-6 pt-5 pb-4 border-b border-white/[0.07] space-y-0 shrink-0">
          <div className="w-8 h-8 rounded-lg bg-violet-500/15 flex items-center justify-center shrink-0">
            {isEdit ? <Pencil size={15} className="text-violet-400" /> : <ShoppingCart size={15} className="text-violet-400" />}
          </div>
          <div>
            <DialogTitle className="text-sm font-semibold tracking-tight text-white/90">{isEdit ? 'Edit Order' : 'Create Order'}</DialogTitle>
          ...
          <form onSubmit={handleSubmit(onFormSubmit)} className="flex flex-col flex-1 overflow-hidden">
            <div className="overflow-y-auto px-6 py-5 flex-1 space-y-5">
              <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                <div className="col-span-2">
                  <SearchableSelectField name="customer_id" label="Customer" options={customerOptions} placeholder="Search customer…" />
                </div>
                <div className="col-span-2">
                  <TextFieldInput name="address" label="Delivery Address" placeholder="123 Main St…" />
                </div>
                <div className="col-span-1">
                  <TextFieldNumber name="delivery_fees" label="Delivery Fees" placeholder="0" />
                </div>
                <div className="col-span-1">
                  <TextFieldNumber name="tax_rate" label="Tax Rate (%)" placeholder="0" />
                </div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[11px] font-medium text-white/40 uppercase tracking-wider">Basket Items</span>
                  <Button
                    type="button"
                ...
                <div className="space-y-3">
                  {fields.map((field, index) => (
                    <div key={field.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-white/25 uppercase tracking-wider">Item {index + 1}</span>
                        <Button
                          type="button"
                      ...
                      <div>
                        <label className="block text-[11px] font-medium text-white/40 mb-1.5 uppercase tracking-wider">Product</label>
                        <Controller
                            ...
                            <div>
                              <SearchableSelect
                                ...
                                onChange={v => {
                                  ...
                                  if (pid) {
                                    setValue(`basket.${index}.price`, getProductPrice(pid));
                                  }
                                }}
                                placeholder="Search product…"
                              />
                              {fieldState.error && <p className="mt-1 text-[10px] text-red-400/80">{fieldState.error.message}</p>}
                            </div>
                          )}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <TextFieldNumber name={`basket.${index}.quantity`} label="Quantity" placeholder="1" />

                        <TextFieldNumber name={`basket.${index}.price`} label="Price" placeholder="0" />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex items-center justify-end gap-2.5 px-6 py-4 border-t border-white/[0.07] shrink-0">
              ...
              <Button
                ...
                {isSubmitting ? <Loader2 size={13} className="animate-spin" /> : isEdit ? <Pencil size={13} /> : <ShoppingCart size={13} />}
