...
export function OrderTable({ orders, total, isLoading, request, products, onRequestChange, onDelete, onEdit }: Props) {
  ...
  const hasActiveFilter = !!(filter.reference || filter.status || filter.date_from || filter.date_to);
  ...
  const columns: ColumnHeader<Order>[] = useMemo(
    () => [
          ...
          return (
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 relative rounded-full overflow-hidden ring-1 ring-white/10 bg-white/5 shrink-0">
                {c?.avatar ? (
                  <Image width={28} height={28} src={c.avatar} alt={c.first_name ?? ''} className="object-cover" />
                ) : (
              ...
              <div className="min-w-0">
                ...
                <div className="text-white/25 text-[10px] truncate max-w-[160px]">{c?.email}</div>
              </div>
            </div>
          );
        },
      },
      {
        id: 'reference',
        label: 'Reference',
        cellRender: row => <span className="font-mono text-violet-300/80 text-xs">{row.reference}</span>,
      },
      {
        id: 'date',
        label: 'Date',
        cellRender: row => <span className="text-white/40 text-[11px] whitespace-nowrap">{formatDate(row.date)}</span>,
      },

      {
        ...
        cellRender: row => (
          <div className="min-w-0">
            <div className="text-white/45 text-[11px] truncate max-w-[140px]">{row.address ?? row.customer?.address ?? '—'}</div>
            {row.customer?.city && <div className="text-white/25 text-[10px]">{row.customer.city}</div>}
          </div>
        ),
      },
      {
...

      {
        id: 'total_ex_taxes',
        label: 'Subtotal',
        cellRender: row => <span className="text-white/35 text-xs tabular-nums">{formatShortNumber(row.total_ex_taxes)}</span>,
      },
      {
        id: 'total',
        label: 'Total',
        cellRender: row => <span className="text-emerald-400 font-medium tabular-nums text-xs">{formatShortNumber(row.total)}</span>,
      },
      {
        id: 'returned',
        label: 'Returned',
        cellRender: row => (
          <span
            className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${
              row.returned ? 'bg-amber-500/15 text-amber-400 border border-amber-500/20' : 'bg-white/5 text-white/25 border border-white/10'
            }`}>
            {row.returned ? 'Yes' : 'No'}
          </span>
        ),
    ...
    <div className="rounded-xl border border-white/[0.07] bg-[#0F0F1C] w-full">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 border-b border-white/6">
        <div className="relative flex-1 max-w-full xl:max-w-xs ">
          ...
          <Input
            ...
            onChange={e => updateFilter({ reference: e.target.value || undefined })}
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-3 px-5 py-3 border-b border-white/6">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-[10px] text-white/20 uppercase tracking-widest mr-1 shrink-0">Status</span>
          <FilterPill label="All" active={!filter.status} onClick={() => updateFilter({ status: undefined })} />
          {(['ordered', 'delivered', 'cancelled'] as OrderStatus[]).map(s => (
            <FilterPill
              key={s}
              label={STATUS_LABELS[s]}
              active={filter.status === s}
              onClick={() => updateFilter({ status: filter.status === s ? undefined : s })}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div>
            <span className="text-[10px] text-white/20 uppercase tracking-widest shrink-0">From</span>
            <DatePicker value={filter.date_from} onChange={iso => updateFilter({ date_from: iso })} placeholder="Start date" className="w-36" />
          </div>
          <div>
            <span className="text-[10px] text-white/20 uppercase tracking-widest shrink-0">To</span>
            <DatePicker value={filter.date_to} onChange={iso => updateFilter({ date_to: iso })} placeholder="End date" className="w-36" />
          </div>
        </div>
      </div>
      ...
      {showSort && (
        <div className="flex items-center gap-2 px-5 py-3 border-b border-white/6 flex-wrap">
          ...
          {SORT_OPTIONS.map(({ key, label }) => (
            <SortBtn key={key} colKey={key} label={label} sortKey={sortField} sortDir={sortDir} onSort={handleSort as any} />
          ))}
        </div>
      )}
      ...
      <div className="overflow-x-auto scrollbar-hide">
        {isLoading ? (
          <div className="py-20 text-center text-white/20 text-xs">Loading orders…</div>
        ) : orders.length === 0 ? (
          <div className="py-20 text-center text-white/20 text-xs">No orders match your filters.</div>
        ) : (
