...
export function ProductTable({ products, total, isLoading, inventories, request, onRequestChange, onDelete, onEdit }: Props) {
  ...
  const columns: ColumnHeader<Product>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cellRender: row => (
          <div className="w-9 h-9 rounded-lg overflow-hidden ring-1 ring-white/10 bg-white/5 shrink-0 flex items-center justify-center">
            {row.thumbnail ? (
              <Image sizes="36px" width={36} height={36} src={row.thumbnail} alt={row.reference} className="w-full h-full object-cover" />
            ) : (
      ...
      },
      {
        ...
        cellRender: row => (
          <div>
            <div className="font-medium text-white/80 leading-none mb-1 whitespace-nowrap text-xs">{row.reference}</div>
            {row.description && <div className="text-[11px] text-white/25 truncate max-w-[200px]">{row.description}</div>}
          </div>
        ),
      },
      {
      ...
      },
      {
        id: 'price',
        label: 'Price',
        cellRender: row => <span className="text-emerald-400 font-medium tabular-nums text-xs">{formatShortNumber(row.price)}</span>,
      },
      {
        id: 'sales',
        label: 'Sales',
        cellRender: row => <span className="font-mono text-white/50 text-xs tabular-nums">{row.sales ?? 0}</span>,
      },
      {
        id: 'width',
        label: 'W × H',
    ...
    <div className="rounded-xl border border-white/[0.07] bg-overlay">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 px-5 py-3 border-b border-white/6">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-[10px] text-white/20 uppercase tracking-widest mr-1 shrink-0">Category</span>
          <FilterPill label="All" active={!filter.inventory_id} onClick={() => updateFilter({ inventory_id: undefined })} />
          {inventories.map(inv => (
            <FilterPill
              key={inv.id}
              ...
              onClick={() =>
                updateFilter({
                  inventory_id: filter.inventory_id === String(inv.id) ? undefined : String(inv.id),
                })
              }
            />
          ))}
      ...
      <div className="w-full overflow-x-auto scrollbar-hide">
        {isLoading ? (
          <div className="py-20 text-center text-white/20 text-xs">Loading products…</div>
        ) : products.length === 0 ? (
          <div className="py-20 text-center text-white/20 text-xs">No products match your filters.</div>
        ) : (
